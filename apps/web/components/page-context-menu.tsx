...
export function PageContextMenu({ content }: PageContextMenuProps) {
    ...
    const getPrompt = (url: string) => {
        return `I’m looking at this obvia documentation: ${url}. Help me understand how to use it. Be ready to explain concepts, give examples, or help debug based on it.`
    }

    const openLink = (assistant: typeof AI_ASSISTANTS[number]) => {
        // Construct the canonical URL regardless of the current environment
        const url = new URL(window.location.pathname, "https://obvia.fun").toString()
